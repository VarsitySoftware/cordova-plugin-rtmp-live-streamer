...
//  Updated by John Weaver for Varsity Software, Inc. on 8/25/2018
// 
...
#define NSNumber @"videoWidth"
#define NSNumber @"videoHeight"
#define NSNumber @"videoBitRate"
#define NSNumber @"videoMaxBitRate"
#define NSNumber @"videoMinBitRate"
#define NSNumber @"videoFrameRate"
#define NSNumber @"videoMaxKeyframeInterval"
#define NSString @"rtmpServerURL"
#define NSString @"buttonTextStart"
#define NSString @"buttonTextStop"
#define NSString @"buttonTextWaiting"
#define NSString @"buttonTextError"
...
- (instancetype)initWithFrame:(CGRect)frame 
		videoWidth:(NSNumber *)strVideoWidth 
		videoHeight:(NSNumber *)strVideoHeight 
		videoBitRate:(NSNumber *)strVideoBitRate 
		videoMaxBitRate:(NSNumber *)strVideoMaxBitRate 
		videoMinBitRate:(NSNumber *)strVideoMinBitRate 
		videoFrameRate:(NSNumber *)strVideoFrameRate 
		videoMaxKeyframeInterval:(NSNumber *)strVideoMaxKeyframeInterval 
		rtmpServerURL:(NSString *)strRTMPServerURL 
		buttonTextStart:(NSString *)strButtonTextStart 
		buttonTextStop:(NSString *)strButtonTextStop
		buttonTextWaiting:(NSString *)strButtonTextWaiting
		buttonTextError:(NSString *)strButtonTextError
{
	if (strRTMPServerURL)
	{
		self.rtmpServerURL = strRTMPServerURL;
	}
	if (strButtonTextStart)
	{
		self.buttonTextStart = strButtonTextStart;
	}
	if (strButtonTextStop)
	{
		self.buttonTextStop = strButtonTextStop;
	}
	if (strButtonTextStop)
	{
		self.buttonTextStop = strButtonTextStop;
	}
	if (strButtonTextWaiting)
	{
		self.buttonTextWaiting = strButtonTextWaiting;
	}
	if (strButtonTextError)
	{
		self.buttonTextError = strButtonTextError;
	}
...
- (UIButton *)closeButton {
    if (!_closeButton) {
        ...
        [_closeButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
			...
			[_self.containerView removeFromSuperview];
			[_self.session.preView removeFromSuperview];
    ...
    if (!_startLiveButton) {
        ...
        [_startLiveButton addBlockForControlEvents:UIControlEventTouchUpInside block:^(id sender) {
            ...
            if (_self.startLiveButton.selected) {
                [_self.startLiveButton setTitle:@"STREAMING!" forState:UIControlStateNormal];
